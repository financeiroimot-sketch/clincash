...
function Form({ id, addAccount, setShowForm }: FormProps) {
  ...
  async function getPlanosContas() {
    ...
    const dataFiltered = data.filter(item => item?.tipoConta === "receita");
  ...
  async function getData() {
    const data: any = await getDataById(id!, "contasReceber");
    if (data) {
  ...
  async function handleSave(values: any) {
    ...
    const result = await updateData(id!, { ...data, comprovante, nomeComprovante }, "contasReceber");
    if (result) {
